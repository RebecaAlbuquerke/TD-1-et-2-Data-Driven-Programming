...
def test_coucher():
    if Modèle.vérifie_disponibilité('nid') == 'libre' and Modèle.lit_état('Tic') == 'repus':
        Contrôleur.coucher('Tic')
    assert Modèle.vérifie_disponibilité('nid') == 'occupé'
    assert Modèle.lit_état('Tic') == 'repus'
    assert Modèle.lit_lieu('Tic') == 'mangeoire'
    Contrôleur.coucher('Tac')
    assert Modèle.lit_état('Tac') == 'affamé'
    assert Modèle.lit_lieu('Tac') == 'litière'
    Contrôleur.coucher('Patrick')
    assert Modèle.lit_état('Patrick') == 'affamé'
    assert Modèle.lit_lieu('Patrick') == 'litière'
    Contrôleur.coucher('Bob')
    assert Modèle.lit_état('Bob') == None
    assert Modèle.lit_lieu('Bob') == None
    assert Modèle.vérifie_disponibilité('nid') == 'occupé'

def test_reveiller():
    if Modèle.lit_état('Pocahontas') == 'endormi':
        Contrôleur.reveiller('Pocahontas')
    assert Modèle.lit_état('Pocahontas') == 'affamé'
    assert Modèle.lit_lieu('Pocahontas') == 'litière'
    Contrôleur.reveiller('Tac')
    assert Modèle.lit_état('Tac') == 'affamé'
    assert Modèle.lit_lieu('Tac') == 'litière'
    Contrôleur.reveiller('Bob')
    assert Modèle.lit_état('Bob') == None
    assert Modèle.lit_lieu('Bob') == None